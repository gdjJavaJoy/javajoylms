...
public class AllAdminListService{
	...
	// 관리자 입력
	public int addAllAdmin(Admin Admin){
		return allAdminListMapper.insertAllAdminList(Admin);
	}

	// 관리자 수정 Form 
	public Subject modifySubject(int subjectNo){
		return AllAdminListMapper.selectSubjectOne(subjectNo);
	}

	// 관리자 수정 Action 
	public int modifySubject(Admin Admin){ 
		return AllAdminListMapper.updateAllAdminList(Admin);
	}
