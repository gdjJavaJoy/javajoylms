      ...
      <div class="flex flex-col flex-1 w-full">
        ...
        <main class="h-full pb-16 overflow-y-auto">
          <div class="container grid px-6 mx-auto">
          	<c:forEach var="curriculumList" items="${curriculumList}">
            <h2 class="my-6 text-2xl font-semibold text-gray-700 dark:text-gray-200">커리큘럼</h2>
            <!-- CTA -->
	            <a class="flex items-center justify-between p-4 mb-8 text-sm font-semibold text-purple-100 bg-purple-600 rounded-lg shadow-md focus:outline-none focus:shadow-outline-purple"
	              href="${pageContext.request.contextPath}/getCurriculumList?subjectNo=${curriculumList.subjectNo}">
	              <div class="flex items-center">
	                <svg class="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 20 20">
	                  <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path>
	                </svg>
	                <span>${curriculumList.curriculumTitle}</span>
	              </div>
	              <span>${curriculumList.subjectNo}번 강좌 커리큘럼 목록 &RightArrow;</span>
	            </a>
            </c:forEach>
            <!-- 과제 상세보기 테이블 -->
              	 ...
              	 <c:forEach var="curriculumList" items="${curriculumList}">
               		<table class="w-full whitespace-no-wrap">
	                    <tr class="text-sm font-semibold tracking-wide text-left text-gray-500 uppercase border-b dark:border-gray-700 bg-gray-50 dark:text-gray-400 dark:bg-gray-800">
	                    	...
	                    	<td class="px-4 py-3 text-sm">${curriculumList.curriculumTitle}</td>
	                    </tr>
	                    <tr class="text-sm font-semibold tracking-wide text-left text-gray-500 uppercase border-b dark:border-gray-700 bg-gray-50 dark:text-gray-400 dark:bg-gray-800">
	                    	<th class="px-4 py-3">사용 언어</th>
	                    	<td class="px-4 py-3 text-sm">${curriculumList.languageName}</td>
	                    </tr>
	                    <tr class="text-sm font-semibold tracking-wide text-left text-gray-500 border-b dark:border-gray-700 bg-gray-50 dark:text-gray-400 dark:bg-gray-800">
	                    	...
	                    	<td class="px-4 py-3 text-sm">${curriculumList.teacherName}</td>
	                    </tr>
	                    <tr class="text-sm font-semibold tracking-wide text-left text-gray-500 border-b dark:border-gray-700 bg-gray-50 dark:text-gray-400 dark:bg-gray-800">
	                    	<th class="px-4 py-3">시작 날짜</th>
	                    	<td class="px-4 py-3 text-sm">${curriculumList.startDay}</td>
	                    </tr>
	                    <tr class="text-sm font-semibold tracking-wide text-left text-gray-500 uppercase border-b dark:border-gray-700 bg-gray-50 dark:text-gray-400 dark:bg-gray-800">
	                    	<th class="px-4 py-3">마감 날짜</th>
	                    	<td class="px-4 py-3 text-sm">${curriculumList.endDay}</td>
	                    </tr>
              		</table> 
	                ...
	                <c:if test="${level eq 1 || level eq 2}">
	              		<a href="${pageContext.request.contextPath}/modifyCurriculum?curriculumNo=${curriculumList.curriculumNo}">커리큘럼 수정</a>
	              		<span>&nbsp | &nbsp</span>
	                    <a href="${pageContext.request.contextPath}/removeCurriculum?curriculumNo=${curriculumList.curriculumNo}">커리큘럼 삭제</a> 
	                </c:if>
	                </span>
              	  </div>
              	 </c:forEach>
