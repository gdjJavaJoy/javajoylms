...
<script>
   ...
   $('#addrBtn').click(function(){
	...
	$('#addBtn').click(function(){
	 if($('#level').val() =="'1'") {
		 if($('#idck').val() =='') {
				Swal.fire('중복검사 할 id를 입력해 주세요');
				focus('#idck');
				return false;
			} else if ($('#id').val() =='') {
				Swal.fire('id 중복검사를 해주세요');
				focus('#idck');
				return false;
			...
			// .뒤에 최소 한글자에서 최대 3글자까지
				Swal.fire("이메일을 정확히 입력하세요");
				$('#memberEmail').focus();
				return false;
			} else if($('#keyword').val() == ''){
				Swal.fire( '주소검색창에 검색할 주소를 입력해주세요');
				$('#keyword').focus();
				return false;
			} else if ($('#memberAddr').val() == '') {
				Swal.fire('주소검색을 해주세요');
				$('#keyword').focus();
				return false;
				 ...
				 if($('#idck').val() =='') {
					...
					// .뒤에 최소 한글자에서 최대 3글자까지
						...
						return false;
							...
							Swal.fire("최종학력을 선택하세요");
							return false;
						 } else if ($('#date').val()== '') {
							 Swal.fire('학원등록일을 선택하세요');
							 return false;
						 } else if($('#keyword').val() == ''){
							 Swal.fire( '주소검색창에 검색할 주소를 입력해주세요');
							 $('#keyword').focus();
