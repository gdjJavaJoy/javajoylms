...
<html :class="{ 'theme-dark': dark }" x-data="data()" lang="en">
  <head>
    ...
    <title>getSubjectDataLlistByPage</title>
    <link
      ...
      <div class="flex flex-col flex-1 w-full">
        ...
        <main class="h-full pb-16 overflow-y-auto">
          <div class="container grid px-6 mx-auto">
            <h2 class="my-6 text-2xl font-semibold text-gray-700 dark:text-gray-200">${loginUser}님의 Subject Data List Page</h2>
            <!-- CTA -->
            <a class="flex items-center justify-between p-4 mb-8 text-sm font-semibold text-purple-100 bg-purple-600 rounded-lg shadow-md focus:outline-none focus:shadow-outline-purple"
              ...
              <span>강좌로 돌아가기 &RightArrow;</span>
            </a>
            
          
            <!-- With avatar -->
       
	           	 <h4 class="mb-4 text-lg font-semibold text-gray-600 dark:text-gray-300">강좌 자료
	         
	            	<c:if test="${level eq 1 || level eq 2}">
	            		<a href="${pageContext.request.contextPath}/addSubjectData?subjectNo=${subjectNo}" class="text-sm" style="float: right;">자료 등록 -></a>
	       			</c:if>
	            </h4>
            ...
            <div class="w-full mb-8 overflow-hidden rounded-lg shadow-xs">
                ...
                <table class="w-full whitespace-no-wrap">
                  <thead>
                    <tr class="text-sm font-semibold tracking-wide text-left text-gray-500 uppercase border-b dark:border-gray-700 bg-gray-50 dark:text-gray-400 dark:bg-gray-800">                 
                      ...
                      <th class="px-4 py-3">등록날짜</th>
                    </tr>
                  </thead>
                  <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                   <c:forEach var="s" items="${list}" varStatus="cnt">
                     <tr onClick="location.href='${pageContext.request.contextPath}/getSubjectDataOne?subjectDataNo=${s.subjectDataNo}'" style="cursor:pointer;" class="text-gray-700 dark:text-gray-400">
						<td class="px-4 py-3 text-sm">${cnt.index+1}</td> 
						<td class="px-4 py-3 text-sm">${s.subjectDataTitle}</td>
						<c:choose>
              ...
              >
               	<form method="get" action="${pageContext.request.contextPath}/getSubjectDataListByPage" name="search">
	                <span class="flex items-center col-span-3">
               		<input name="subjectNo" class="form-control" type="hidden" value="${subjectNo}">
	                <input name="searchName" class="form-control" type="text"  placeholder="제목 검색">
	                 	  <button type="submit" class="grid px-4 py-3 text-sm">검색</button>
	                 	  <a href="${pageContext.request.contextPath}/getSubjectDataListByPage?subjectNo=${subjectNo}">초기화</a>
	                </span>
	            </form> 
                ...
                <span class="flex col-span-4 mt-2 sm:mt-auto sm:justify-end">
                   ...
                   <a href="${pageContext.request.contextPath}/getSubjectDataListByPage?subjectNo=${subjectNo}">이전</a>
                 </c:if>
                <span>&nbsp  &nbsp</span>
                 <c:if test="${currentPage < lastPage}">
                   <a href="${pageContext.request.contextPath}/getSubjectDataListByPage?subjectNo=${subjectNo}">다음</a> 
                 </c:if>
                </span>
              </div>
            </div>
