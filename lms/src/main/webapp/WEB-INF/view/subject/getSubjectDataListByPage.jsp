			...
			<main class="h-full pb-16 overflow-y-auto">
				<div class="container grid px-6 mx-auto">
                  ...
                  <path
						...
						</div> <span>강좌로 돌아가기 &RightArrow;</span>
					</a>
					...
					<h4
						...
						<c:if test="${level eq 1 || level eq 2}">
							<a
								href="${pageContext.request.contextPath}/addSubjectData?subjectNo=${subjectNo}"
								class="text-sm" style="float: right;">자료 등록 -></a>
						</c:if>
					</h4>
					...
					<div class="w-full mb-8 overflow-hidden rounded-lg shadow-xs">
							...
							<table class="w-full whitespace-no-wrap">
								...
								<tbody
									...
									<c:forEach var="s" items="${list}" varStatus="cnt">
										<tr
											onClick="location.href='${pageContext.request.contextPath}/getSubjectDataOne?subjectDataNo=${s.subjectDataNo}&subjectNo=${subjectNo}'"
						...
						<div
							...
							<form method="get"
								action="${pageContext.request.contextPath}/getSubjectDataListByPage"
								name="search">
								<span class="flex items-center col-span-3"> <input
									name="subjectNo" class="form-control" type="hidden"
									value="${subjectNo}"> <input name="searchName"
									class="form-control" type="text" placeholder="제목 검색">
									<button type="submit" class="grid px-4 py-3 text-sm">검색</button>
									<a
									href="${pageContext.request.contextPath}/getSubjectDataListByPage?subjectNo=${subjectNo}">초기화</a>
								</span>
							</form>
							...
							<span class="flex col-span-4 mt-2 sm:mt-auto sm:justify-end">
								<c:if test="${currentPage > 1}">
									<a
										href="${pageContext.request.contextPath}/getSubjectDataListByPage?subjectNo=${subjectNo}">이전</a>
								</c:if> <span>&nbsp &nbsp</span> <c:if test="${currentPage < lastPage}">
									<a
										href="${pageContext.request.contextPath}/getSubjectDataListByPage?subjectNo=${subjectNo}">다음</a>
								</c:if>
							</span>
						</div>
					</div>
